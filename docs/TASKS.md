...
- 验收日期：2026-09-21
- 验收对象：B03 管理员用户列表后端接口与 `/users` 前端页面。
- 结论：**验收通过**。

Validation:

- 后端 `PYTHONPATH=. ./.venv/bin/pytest -q`：44 passed，3 skipped。
- 前端 `npm test -- --run`：12 个测试文件、71 项测试通过。
- 前端 `npm run build`：通过；`npm run lint`：通过。
- `GET /api/v1/admin/users` 仅允许管理员访问；普通用户返回 403，未认证请求返回 401。
- 后端支持关键词、等级、状态、管理员类型、分页及白名单排序；关键词会转义 SQL LIKE 通配符。
- 返回字段包含用户列表页面所需信息，不包含密码摘要或 token。
- 前端 `/users` 使用真实接口，支持筛选、分页、排序、URL 参数恢复、空状态、无结果状态、失败重试及字段展示。
- 测试覆盖管理员权限、筛选、分页、安全字段、通配符转义、排序白名单和前端重试流程。

### Review 2

- 验收日期：2026-09-21
- 验收对象：B03 真实运行环境中的 `/users` 用户列表请求。
- 结论：**待修改**。

Finding:

#### R1 / P1：真实联调的用户列表 GET 接口返回 405

- 触发请求：`GET /api/v1/admin/users?sort=created_at&order=desc&limit=20&offset=0`。
- 实际结果：`405 Method Not Allowed`。
- 影响：`/users` 页面无法读取真实用户列表，B03 的核心垂直流程未完成；前端单元测试和后端 TestClient 测试不能替代运行中的前后端联调。
- Required Change：确认运行中的 API 已部署包含 B03 的 `GET /api/v1/admin/users` 路由，并确认 Vite 代理指向该 API；重启/重建服务后，用真实管理员会话复验 200 响应、列表字段、筛选、分页和排序。若仍返回 405，检查 API 版本或路由方法是否与前端一致。

备注：消息中的 `Unsupported URL scheme ;` 是浏览器复制多条 fetch 时附带分号产生的命令格式问题；真正阻断 B03 的证据是 Network 面板中的 405。

安全备注：你贴出的 Bearer token 已暴露，验收后应立即退出该账号或撤销该 token，并重新登录生成新 token。

### Review 3

- 验收日期：2026-09-21
- 验收对象：B03 修复后的真实前后端联调。
- 结论：**验收通过**。
- 依据：用户已确认实际运行环境中 `GET /api/v1/admin/users` 请求已恢复正常，`/users` 页面可正常加载用户列表；此前 Review 2 的 405 问题视为已解决。
- 保留验证：后端 44 passed、3 skipped；前端 71 passed；构建和 lint 通过。
- 备注：Review 2 的 405 历史记录保留，不影响本次人工复验结论。
...
- 裁决日期：2026-09-21
- 裁决结论：B04 可先实现用户创建核心闭环，不等待 D01，也不得在 B04 内复制建设审计基础设施。
- 依赖处理：验收标准中的“创建操作写入操作日志”依赖 D01。D01 完成 `audit_logs` 表、记录服务和权限边界后，必须回接 B04 的创建操作并补做联验；在该联验完成前，B04 不得宣称全部验收通过。
- 原因：当前 `manabi_api` 没有 `audit_logs` 表或审计代码，审计基础设施明确属于里程碑 D / D01。提前扩展 B04 会改变任务边界并重复 D01 的设计。
- 本裁决不豁免安全要求：B04 实现不得记录密码或 token；D01 接入时必须记录创建行为且只保存安全摘要。

### Review 1

- 验收日期：2026-09-21
- 验收对象：B04 管理员创建普通用户的后端接口与 `/users` 前端创建流程。
- 结论：**核心功能通过；整体待 D01 审计联验**。

Validation:

- 后端 `PYTHONPATH=. ./.venv/bin/pytest -q`：51 passed，3 skipped。
- 前端 `npm test -- --run`：12 个测试文件、76 项测试通过。
- 前端 `npm run build`：通过；`npm run lint`：通过。
- 后端覆盖管理员权限、普通用户创建、强制 `is_admin=false`、密码和用户名校验、默认等级、显示名称清洗、重复用户名 409 及敏感字段不返回。
- 前端已提供创建用户弹窗、表单校验、创建成功刷新列表、重复用户名提示、服务端错误提示和防重复提交。
- 按此前范围裁决，创建操作写入审计日志尚未验收；当前项目仍没有 D01 的 `audit_logs` 基础设施。

Finding:

#### R1 / P1（依赖项）：创建操作尚未接入审计日志

- 当前实现满足 B04 的用户创建核心闭环，但无法满足最终验收标准“创建操作写入操作日志”。
- Required Change：待 D01 建立审计基础设施后，将 `POST /api/v1/admin/users` 接入审计记录，验证不记录密码或 token，并追加 B04 联验记录。

Next:

- B04 保持“待修改”，但无需阻塞当前核心功能继续开发；D01 完成后补做审计联验，届时再决定是否改为“验收通过”。
...
- 验收日期：2026-09-23
- 验收对象：B05 用户详情与学习摘要后端接口及 `/users/{user_id}` 前端页面。
- 结论：**验收通过**。

Validation:

- 后端 `PYTHONPATH=. ./.venv/bin/pytest -q`：56 passed，3 skipped。
- 前端 `npm test -- --run`：13 个测试文件、80 项测试通过。
- 前端 `npm run build`：通过；`npm run lint`：通过。
- 两个详情接口均要求管理员权限；未认证返回 401，普通用户返回 403。
- 不存在的用户在详情和统计接口均返回 404。
- 详情仅返回用户资料字段，不包含密码摘要或 token。
- 无练习数据时返回零值和空等级列表；有练习数据时按等级汇总练习次数、答题数、正确数、正确率和未解决错题数。
- 统计响应不包含答案快照、正确答案内部字段等敏感练习内容。
- 前端已接入真实接口，支持从用户列表进入详情、资料展示、学习摘要、各等级数据、空数据状态、错误重试及返回列表。
...
- 裁决日期：2026-09-23
- 审计依赖：与 B04 一致，“修改前后写入操作日志”延后到 D01。D01 完成审计基础设施后，必须回接 B06 的 PATCH 操作并补做联验；B06 不得自行复制建设 `audit_logs`。
- 并发冲突裁决：B06 本阶段必须实现 409 并发保护。使用 B01 已存在的 `updated_at` 作为乐观锁版本：详情响应返回该字段，PATCH 携带期望值，服务端在事务内比较当前值；不一致返回 409，成功更新后写入新的 `updated_at`。不得用无条件覆盖代替并发检查，也不新增独立审计或版本基础设施。
- 范围边界：管理端 PATCH 只允许 `display_name`、`username` 和并发校验字段；不提供 JLPT 等级编辑能力。不得修改 `level`、`is_admin`、`status`、密码或 token。用户名冲突应返回明确的 409。
- 结论：Worker 可按上述边界开始实现；在 D01 回接完成前，B06 不能宣称全部验收通过。

### Review 1

- 验收日期：2026-09-23
- 验收对象：B06 管理员编辑用户资料的后端 PATCH 接口与用户详情页编辑流程。
- 结论：**核心功能通过；整体待 D01 审计联验**。

Validation:

- 后端 `PYTHONPATH=. ./.venv/bin/pytest -q`：64 passed，3 skipped。
- 前端 `npm test -- --run`：13 个测试文件、84 项测试通过。
- 前端 `npm run build`：通过；`npm run lint`：通过。
- PATCH 仅允许 `username`、`display_name` 和 `updated_at` 并发校验字段；尝试提交 `level`、`is_admin`、`status`、密码等字段会被拒绝。
- 管理员权限、用户不存在 404、用户名冲突 409、用户名/显示名称清洗及成功后登录行为均已测试。
- 使用 `updated_at` 乐观锁检测并发修改，旧版本返回 409 `EDIT_CONFLICT`，且不会覆盖第一次修改。
- 前端编辑表单仅展示用户名和显示名称，保存后更新详情并失效列表缓存；冲突和用户名占用均保留弹窗并给出明确提示。

Remaining:

- “修改前后写入操作日志”按范围裁决延后至 D01。当前尚无审计基础设施，因此 B06 暂不能标记为最终验收通过。
- D01 完成后需回接 PATCH 操作，确认日志不包含密码或 token，再追加 B06 联验。
...
- 裁决日期：2026-09-23
- 角色映射：当前数据库只有 `is_admin`，没有独立的 `super_admin` 字段；本阶段将 `is_admin=true` 的管理员视为受保护管理员。
- 禁用自己：服务端直接拒绝当前管理员禁用自己的请求，返回稳定错误码，例如 `CANNOT_DISABLE_SELF`。
- 最后管理员保护：在同一数据库事务内，先按固定顺序锁定所有 `status='active' AND is_admin=true` 的管理员行，再判断目标。如果目标是管理员且锁定后的管理员数量小于等于 1，拒绝操作并返回稳定错误码 `LAST_ADMIN_PROTECTED`。
- 普通用户：不需要执行最后管理员计数，但仍需锁定目标用户、设置 `status='disabled'` 和 `disabled_at`，并撤销该用户全部 token。
- 并发要求：计数、状态变更和 token 撤销必须在同一事务内完成；不能先查询计数、之后再单独更新，也不能只依赖前端判断。所有调用路径应保持一致的锁定顺序，避免两个管理员并发禁用造成竞态。
- D01 依赖：操作日志继续延后到 D01；B07 不提前建设审计基础设施，但 D01 完成后必须回接禁用和启用操作。
- 初始化管理员：可以通过部署/服务器侧的 `python -m scripts.set_admin USERNAME` 显式创建或提升一个管理员；该命令交互式读取密码、撤销旧 token 并保留学习数据。不得在 migration、seed 文件或代码中写入固定用户名/密码，也不在当前 B07 引入独立 `super_admin` 字段。

### Review 1

- 验收日期：2026-09-23
- 验收对象：B07 管理员禁用/启用用户的后端接口与用户详情页操作流程。
- 结论：**核心功能通过；整体待 D01 审计联验**。

Validation:

- 后端 `PYTHONPATH=. ./.venv/bin/pytest -q`：72 passed，3 skipped。
- 前端 `npm test -- --run`：13 个测试文件、88 项测试通过。
- 前端 `npm run build`：通过；`npm run lint`：通过。
- 禁用普通用户会设置 `disabled_at`、撤销全部 token，并阻止旧 token 访问和重新登录；启用后恢复登录。
- 服务端拒绝禁用自己，返回 `CANNOT_DISABLE_SELF`；最后一个 active 管理员受 `LAST_ADMIN_PROTECTED` 保护；保留其他管理员时可禁用目标管理员。
- 禁用/启用均要求管理员权限，未知用户返回 404；前端包含原因输入、二次确认、状态刷新和稳定错误提示。
- 禁用原因当前仅作为请求字段传递，未落库；这是因为 D01 审计基础设施尚未实现。

Remaining:

- “操作写入日志”按范围裁决延后至 D01。D01 完成后需回接禁用和启用操作，保存安全摘要且不记录密码或 token，再追加 B07 联验。
...
- 验收日期：2026-09-23
- 验收对象：B08 管理员重置密码、撤销用户全部会话的后端接口与用户详情页操作流程。
- 结论：**核心功能通过；整体待 D01 审计联验**。

Validation:

- 后端 `PYTHONPATH=. ./.venv/bin/pytest -q`：79 passed，3 skipped。
- 前端 `npm test -- --run`：13 个测试文件、92 项测试通过。
- 前端 `npm run build`：通过；`npm run lint`：通过。
- 重置密码后旧密码失效、新密码生效，目标用户全部旧 token 失效。
- 撤销全部会话不修改密码，旧 token 失效。
- 两个接口均要求管理员权限，未知用户返回 404，已删除用户拒绝操作。
- 密码最小长度、确认密码、强度提示、二次确认及错误提示已覆盖；响应不返回密码摘要、token 或输入密码。
- 前端重置密码和撤销会话按钮已接入真实接口；重置密码不会回显旧密码。

Remaining:

- “操作有审计日志”按此前范围原则延后至 D01。当前项目没有审计基础设施，因此 B08 暂不能标记为最终验收通过；D01 完成后需回接两项操作并确认日志不保存密码或 token。
